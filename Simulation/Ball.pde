...
//Constructors  
  Ball()
  {
    ...
    velx = gen.j*distance_from_target+gen.l*distance_from_target;                     //the speed vector is built using the distance between the ball and the target (adjusted with coefficients)
    vely = gen.k*distance_from_target+gen.m*distance_from_target;
    vel = new PVector(velx,vely);
    

  }
  
  Ball(Time t, float op, PVector tgt)
  {
    dead = false;
    fitness = 0;
    original_pos = op;
    ...
    gen = new Genome();
    distance_from_target = dist(pos.x,pos.y,target.x,target.y);
    velx = gen.j*distance_from_target+gen.l*distance_from_target;
    vely = gen.k*distance_from_target+gen.m*distance_from_target;
    vel = new PVector(velx,vely);
    
  }
  
   Ball(Genome g, Time t, float op, PVector tgt)
  {
    dead = false;
    fitness = 0;
    original_pos = op;
